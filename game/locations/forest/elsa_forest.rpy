...
label elsa_first_time_blessing:
    ...
    menu:
        ...
        "Конечно! Магия — это когда чертишь круги и читаешь заклинания.":
            ...
            mind "Я сжимаю губы. Кажется, все мои представления о магии только что были растоптаны в ледяной кашу."  
        ...
        "Магия? Это что-то про зелья?":
            ...
            mind "Отлично. Теперь я ещё и невежественный деревенщина. Ну, день определённо идёт в гору."  

    e "Так, ясно. Ты полнейший профан."
    e "Попробуем ещё раз."

    "Её голос звучит так, будто она уже потеряла надежду."
    mind "Это обидно, но если я сейчас сдамся — точно оставлю о себе впечатление беспросветного глупца."  
    ...
    menu:
        ...
        "Создам барьер и пережду.":
            ...
            show e smile_closed_eyes with dissolve
    ...
    mind "Что?.. Куда? Я не подписывался на это! "  
    ...
    e "Не тормози, пойдем сразу к наставнице, пускай она тебя оценит."
...
label elsa_first_time_no_blessing:
    ...
    "Что-то в её взгляде настораживает. Похоже, я совсем не произвёл впечатления, на которое надеялся."  
    ...
    menu:
        e "От тебя исходит слабая аура, но ты не чувствуешь магию?"
    ...
    "В голове вертится миллион мыслей. Если я облажаюсь, [e.name], похоже, не даст мне второго шанса. Но если я покажу хоть что-то, что она не ожидает... это будет шанс доказать, что я не такой уж полный ноль."  
    ...
    menu:
        ...
        "Я бы хотел узнать, где здесь библиотека" if not canVisit("lib"):
            ...
            if not canVisit("mt"):
                ...
                e "Но не в магическую башню, до нее ты просто так не доберешься, сначала ты должен доказать, что достоин!"
                p "Тогда хотя бы расскажи, как добраться до библиотеки..."
                e "Пойдём, я покажу тебе дорогу. Только не потеряйся."
            else:
                e "Ах, тебе интересно? Хорошо, я покажу тебе путь."
...
label go_to_lib_with_elsa:
    ...
    $ updateCanVisit("lib", True)
    e "А за городом есть магическая башня. Если хочешь получить еще больше знаний, тебе нужно идти туда."
