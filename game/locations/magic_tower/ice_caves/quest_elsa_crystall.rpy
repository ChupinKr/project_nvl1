...
label quest_elsa_crystall_start:
    ...
    scene bg ice_cave_golem1 at bg_size with fade
    p "!!!!"
    ...
    scene bg ice_cave_main at bg_size with fade
    emilia "Кто нибудь! Помогите мне!"
    "Ты слышишь, как кто-то зовет на помощь."
    scene bg emilia_captured at bg_size with fade
...
label free_emilia:
    ...
    if strength > 50:
        ...
        $customNotify("Необходимо 50 силы или 50 интеллекта")
    ...
    scene bg ice_caves_crystall at bg_size with fade
    ...
    scene bg emilia_kiss1 at bg_size with fade
    pause .5
    scene bg emilia_kiss2 at bg_size with dissolve
    call hide_dialog
    "Она внезапно наклоняется и мягко целует тебя в щёку, её губы прохладны, но нежны."
    scene bg emilia_kiss3 at bg_size with fade
    call hide_dialog
    scene bg ice_caves_crystall_empty at bg_size with fade
...
label deal_with_emilia:
    ...
    scene bg emilia_captured_blush at bg_size with dissolve
    call hide_dialog
    ...
    scene bg emilia_sex_chains1 at bg_size with fade
    call hide_dialog
    ...
    scene bg emilia_sex_chains2 at bg_size with fade
    call hide_dialog
    ...
    scene bg emilia_sex_chains2_1 at bg_size with fade
    call hide_dialog

    scene bg emilia_sex_chains2_2 at bg_size with dissolve
    call hide_dialog
    "Ты прижимаешь её к холодной земле, кладя свой член на её ягодицы."

    scene bg emilia_sex_chains2_3 at bg_size with dissolve
    call hide_dialog
    emilia "Ч-что это? Это... твой член? Он такой горячий..."
    p "Скоро узнаешь."

    scene bg emilia_sex_chains3 at bg_size with dissolve
    call hide_dialog
    ...
    scene bg emilia_sex_chains4 at bg_size with dissolve
    call hide_dialog
    ...
    scene bg emilia_sex_chains4_1 at bg_size with dissolve  
    pause .5
    scene bg emilia_sex_chains4_2 at bg_size with dissolve  
    pause .5
    scene bg emilia_sex_chains4_3 at bg_size with dissolve  
    pause .3
    scene bg emilia_sex_chains4_2 at bg_size with dissolve 
    pause .3
    scene bg emilia_sex_chains4_1 at bg_size with dissolve 
    ...
    scene bg emilia_sex_chains4_2 at bg_size with dissolve 
    pause .1
    scene bg emilia_sex_chains4_3 at bg_size with flash  
    call hide_dialog

    scene bg emilia_sex_chains5_cum1 at bg_size with flash  
    call hide_dialog
    "Ты достигаешь предела, и поток спермы вырывается внутрь неё. [emilia.name] издаёт громкий стон."
    
    scene bg emilia_sex_chains5_cum2 at bg_size with flash  
    pause .5
    scene bg emilia_sex_chains5_cum3 at bg_size with flash  
    emilia "Ааааах!"

    p "Ещё чуть-чуть."
    scene bg emilia_sex_chains5_cum4 at bg_size with flash  
    "Ты выпускаешь ещё один мощный заряд спермы в её киску."
    
    scene bg emilia_sex_chains6_cum1 at bg_size with dissolve  
    "Сперма стекает ручьями по её бёдрам, а её лицо выражает экстаз."

    scene bg emilia_sex_chains6_cum2 at bg_size with dissolve  
    "Ты вытаскиваешь член, и последние капли спермы падают на её спину."

    scene bg emilia_sex_chains_after at bg_size with fade
    ...
    menu:
        ...
        "Я солгал":
            ...
            scene bg emilia_sex_chains_after2 at bg_size with fade
