...
label quest_elsa_crystall_start:
    ...
    # [emilia.name] в цепях, её лицо выражает усталость, но глаза полны решимости
    ...
    emilia "Меня зовут [emilia.name]... Я пыталась взять кристалл, чтобы усилить свою магию, но эти големы схватили меня."
...
label free_emilia:
    ...
    # [emilia.name] свободна, её лицо озаряется благодарной улыбкой
    ...
    show emilia freed at center with dissolve
    emilia "Ещё раз спасибо... Я не знаю, как отблагодарить тебя."
...
label deal_with_emilia:
    ...
    emilia "Взамен? Я... у меня нет ничего ценного здесь."
    p "Тогда, может, ты сама станешь моей наградой?"
    "[emilia.name] краснеет, её глаза расширяются от удивления, но она быстро отводит взгляд."
    emilia "Ты... серьёзно? Я не уверена, что это правильно, но..."
    emilia "Если ты меня освободишь, я... соглашусь. Это мой выбор."
    menu:
        "Принять её согласие":
...
label emilia_sex_scene:
    ...
    show emilia freed at center with dissolve
    emilia "Спасибо... Я обещала, и я сдержу слово."
    ...
    # [emilia.name] стоит близко, её одежда слегка приспущена, открывая плечи
    emilia "Я... не очень опытна в этом, но я хочу, чтобы ты знал — это моё решение."
    p "Ты уверена?"
    emilia "Да, [hero_name]. Я доверяю тебе."
    scene bg emilia_sex2 at Transform(zoom=1.5) with dissolve
    # Она опускается перед тобой, её руки касаются твоей одежды
    "[emilia.name] медленно расстёгивает твою одежду, её пальцы дрожат, но она не отступает."
    ...
    emilia "Ах... это... теплее, чем я ожидала..."
    scene bg emilia_sex4 at Transform(zoom=1.5) with flash
    # Кульминация: ты кончаешь, [emilia.name] лежит рядом, её лицо выражает смесь смущения и облегчения
    "Ты достигаешь предела, и [em.name] мягко опускается на холодный пол, её кожа покрыта лёгким румянцем."
    emilia "Ты... сделал это. Теперь кристалл твой."
    scene bg ice_caves_crystal with fade
    "Она указывает на кристалл, и ты забираешь его."
    show emilia freed at center with dissolve
    emilia "Я не забуду тебя, [hero_name]. Будь осторожен."
    p "Ты тоже."
    "[emilia.name] уходит в глубину пещер, а ты отправляешься обратно."
    jump elsa_quest_ice_resonance_reward
