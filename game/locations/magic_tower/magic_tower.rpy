...
label magic_tower_first_visit:
    ...
    menu:
        mer "Что является основой любого магического заклинания?"
        "Магическая палочка?":
    ...
    menu:
        mer "Чем отличается огненная магия от воздушной?"
    ...
    menu:
        mer "Попробуй создать шар света."
    ...
    mer "Ах да, самое главное, в магическую башню можно попасть только вечером, я не собираюсь отключать барьер ради тебя."
...
label magic_tower_with_elsa_no_blessing:
    ...
    menu:
        mer "Ты хочешь обучаться магии?"
        "Да":
            mer smile "Вопрос решен. Пока подумай обо всем. Видимо тебе нужно время." with dissolve
            mer "Кстати зайти ты сможешь лишь по вечерам, я не собираюсь отключать барьер ради тебя."
            $ updateCanVisit("mt", True)
            jump city
        "Нет":
...
label magic_tower_alone:
    if not isEvening():
        "В магическую башню получится попасть лишь вечером из-за барьера"
        if intelligence > 50:
            "Но ты смог преодолеть барьер поставленный [mer.name] и спокойно прошел"
        else:
