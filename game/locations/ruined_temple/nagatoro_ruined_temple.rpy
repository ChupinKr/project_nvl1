...
label visit_nagatoro_ruined_temple:
    ...
    if first_time_nagatoro:
        ...
        nag "Ну что, мальчик, заблудился или ты нарочно пришёл проверить, насколько я опасна?"
        ...
        show nag neutral
        nag "Так что давай разберёмся. Ты – наивный путешественник или кто-то, способный меня развлечь?"
        ...
        menu:
            "Дать [nag.name] понять, кто тут главный":
                ...
                nag "Вот так мне нравится! Давай, покажи, на что ты способен! Но помни, жалеть себя потом не стану."
                call start_battle(100, nag_str, nag.name, battle_location_ruined_temple)
            "Не связываться":
                p "Сегодня не время для сражений. Мне надо подумать."
                show nag grin
                nag "Как скажешь, умник. Но помни: я не забываю тех, кто меня заинтересовал, и если захочешь ещё поиграть – я всегда тут."
                nag "В следующий раз я точно не позволю тебе уйти так легко."
                "Я понимаю, что, возможно, стоило бы убежать, но её взгляд заставляет меня оставаться."
...
label battle_win_ruined_temple_nagatoro:
    ...
    if not can_visit_guild:
        ...
        show nag grin
        nag "Хех. Конечно нет. Ты меня заинтересовал."
        ...
        # Если can_visit_guild уже установлен, значит, ты уже доказал свою стойкость, и я просто не хотела тебя сломать.
        p "Почему ты сразу не предложила пойти в Гильдию? Ты что, боялась, что я сломаюсь?"
        show nag neutral
