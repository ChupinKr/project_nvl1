...
label talk_miku_drinks_menu:
    ...
    menu:
        "Энергетический эль (10 золота)":
            miku "Этот эль действительно придаст тебе сил!"
            if money >= 10:
        ...
        "Магический ликёр (10 золота)":
            miku "Опьяняет и слегка пробуждает магическое чутье!"
            if money >= 10:
        ...
        "Чёрный ром (10 золота)":
            miku "Тяжелый черный ром, после него все твое тело скажет \"Спасибо\"!"
            if money >= 10:
...
label talk_miku_info:
    if miku_love >= 10 and not can_visit_library:
        miku "Знаешь, ты неплохой! "
        miku "Ладно, расскажу тебе одну тайну... "
        miku "В городе есть **тайная библиотека**, где можно найти запрещённые магические знания!"
        miku "*Описание того, как найти библиотеку*"
        $ can_visit_library = True
    elif miku_love >= 20 and not can_visit_training_ground:
        miku "Ты мне нравишься!"
        miku "Дам тебе наводку: неподалёку есть место, где любой желающий может стать сильнее."
        miku "*Описание того, как попасть на тренировочную площадку*"
        miku "Если хочешь стать сильнее – тебе туда!"
        $ can_visit_training_ground = True
    elif miku_love >= 30 and not can_visit_bar:
        miku "Ты определённо мой типаж!"
        miku "В городе есть **закрытый бар**, куда пускают только своих."
        miku "Там можно найти очень... полезных знакомств!"
        miku "*Описание того, как найти закрытый бар*"
        miku "Скажи, что ты от меня и тебя пропустят."
        $ can_visit_bar = True
    elif miku_love >= 50:
        miku "Мне нравится, что ты не просто клиент."
        miku "Я могу показать тебе нечто особенное... но только если ты готов к этому~"
        menu:
            "Я готов":
