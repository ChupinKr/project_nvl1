...
label guild_d_menu:
    "Ты находишь [d.name]"
    if my_darkness_love >= 50:
        show d smile_shy with dissolve
    else:
        show d neutral with dissolve
    menu:
        "А где тут у вас туалет?" if not first_root_d and my_darkness_love >= 80:
            jump toilet
        "Тебя пора наказать" if not first_root_d:
            jump guild_d_root_menu
        "Научи меня":
            call start_battle(my_darkness_str, 60, d.name, 'return_to_this')
            if last_battle_win:
                d @smile_shy "А-а-аах, ты победил меня, великую воительницу~"
                $addLove(my_darkness, 10)
                $addChar(["str"], 5)
                $addNPCStr(my_darkness,20)
                if my_darkness_love >= 50:
...
label toilet:
    ...
    menu:
        "Позвать [d.name]" if my_darkness_love >= 90:
            jump toilet_root_d
        "Позвать [nag.name]" if my_nag_love >= 90:
            jump toilet_root_nag
        "Позвать [d.name] и [nag.name]" if my_nag_love >= 100 and my_darkness_love >= 100:
